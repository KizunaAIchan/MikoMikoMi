...
public class InputFieldMod : UIBehaviour {
    ...
    void Update() {
		...
		if (!caret) {
            if (caretTransform == null)
			    caretTransform = InputField.transform.Find(InputField.transform.name + " Input Caret");
			if (caretTransform) {
                if(graphic == null)
                    graphic = caretTransform.GetComponent<Graphic>();
				if (!graphic) {
	...
	void ResizeInput(string text) {
		...
		else defaultHeight = ((textGenerator.GetPreferredHeight("", settings) * textRows ) / ScaleFactor) + VerticalOffset;
